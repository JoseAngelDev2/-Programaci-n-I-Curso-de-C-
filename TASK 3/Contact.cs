...
void formContact()
{
    ...
    name = VerifyFieldString("name", "Ana");
    lastname = VerifyFieldString("lasname", "Grabiel");
    address = VerifyFieldString("address", "13 main street SC 91000");
    email = VerifyFieldString("email", "ana@dominio.com");
    phone = VerifyFieldString("phone", "XXX-XXX-XXXX");
    age = verifyAge("age");
    isBestFriend = MultiOptionList.Contains(VerifyFieldString("option Isbesfriend [YES OR NO]", "yes"));
...
static string VerifyFieldString(string Typeinput, string TypeExample)
{
    bool active = true;
...
static string verifyLargeOfCharater(string input, string Typeinput, string TypeExample)
{
...
void SearchContact()
{
    ...
    ShowAllContact(ids, names, lastnames, addresses, telephones, emails, ages, bestFriends);
    Console.Write("\nWrite the Contacts number or id you would like to search: ");

    string userSearch = Console.ReadLine()!; // 1
    ...
    {
        Console.ForegroundColor = ConsoleColor.Green;
        Console.WriteLine($"The list has {telephones.Count} registered Contacts ❎");
        Console.ResetColor();
        return;
    }
    else if (foundPhone.Key == userSearch)
    {
        ...
        Console.WriteLine("\n✅ USUARIO ENCONTRADO ✅\n");
        Console.ResetColor();
        ShowContactFound(userSearch, 0);
        Console.ReadKey();
    }
    else if (int.TryParse(userSearch, out int id))
    {
        if (names.ContainsKey(id))
        {
            ShowContactFound(string.Empty, id);
        }
    }
}
...
void ModifyContact()
{
    ...
    else if (telephones.ContainsValue(optionNumber)) 
    {
        Console.ForegroundColor = ConsoleColor.Yellow;
...
void deleteContact()
{
    ...
    Console.Write("Write the number or id: ");
    string optionNumber = Console.ReadLine()!;

    if (telephones.Count == 0)
    {
        Console.WriteLine($"The list has {telephones.Count} registered Contact ❎");
    }
    else if (telephones.ContainsValue(optionNumber))
    {
        int IDNumber = telephonesInverted[optionNumber];
        ShowMessageConfirmRemove(optionNumber);
        ...
        else
        {
            ...
            Console.WriteLine("The contact not was removed");
            Console.ReadKey();
            Console.ResetColor();
        }
    }
    ...
    else if (int.TryParse(optionNumber, out int id))
    {
        if (names.ContainsKey(id))
        {
            optionNumber = telephones[id];
            Console.WriteLine(optionNumber);
            Console.ReadKey();
            ShowMessageConfirmRemove(optionNumber);
            bool optionBool = MultiOptionList.Contains(Console.ReadLine()!.ToLower());
            if (optionBool) { RemoveContact(id, optionNumber); }
        }
    }

}

void ShowMessageConfirmRemove(string optionNumber)
{
    Console.WriteLine("\n❎ you sure the delete this contact ❎\n");
    ShowContactFound(optionNumber, 0);
...
void ShowContactFound(string optionNumber, int id)
{
...
static int SearchIdOrPhoneNumber(string optionNumber, int id, Dictionary<string, int> telephonesInverted) // Search Contact for number or ID
{
    int phone = 0;

    if (!string.IsNullOrEmpty(optionNumber))
        phone = telephonesInverted[optionNumber];

    int IDNumber = optionNumber == string.Empty ? id : phone;
    return IDNumber;
}
